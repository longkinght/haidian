...
方案建立8维度结构化风险矩阵 [data:risk.json]，覆盖数据隐私、实施复杂度、公众接受度、运维成本、政策不确定性、空间争议、技术成熟度和公平与包容性。其中实施复杂度和政策不确定性评分最高（4分），已标注人工复核要求。其余维度评分3分，均设有缓解措施。风险矩阵与 `simulation.json` 中的风险验证任务联动，确保风险评分与方案表述一致 [data:simulation.json#SIM-RISK-01]。
