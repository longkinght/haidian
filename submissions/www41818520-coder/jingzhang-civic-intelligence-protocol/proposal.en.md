...
The V3 revision compresses the design into four handover-ready outputs: **one six-step civic-intelligence protocol, one spatial prototype of an “ordinary mainline + AI test siding”, three first-issue implementation contracts and seven public rights**. The six stages SENSE—ASK—PLAN—ACT—VERIFY—LEARN are each tied to a spatial carrier, accountable human, ordinary equivalent, evidence output and stop condition. The contracts bind each key area to a suggested lead, prerequisites, cost band, phases, indicators, operating gaps and exit condition. The rights make no-device access, anonymity, data exit, human review, status information, appeal and accessibility enforceable spatial rules rather than slogans. [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:phasing_implementation]
