...
本次 V3 深化把方案压缩为四个可交接成果：**一套六步市民智能协议、一个“普通主线 + AI 试验支线”的空间原型、三份首发实施合同、七项公共权利**。六步协议把 SENSE—ASK—PLAN—ACT—VERIFY—LEARN 分别绑定到空间载体、人工责任、普通等价路径、证据输出和停止条件；三份合同分别绑定三处重点区的牵头建议、审批前置、成本等级、阶段、指标、运维缺口与退出条件；七项权利保证无设备、匿名、退出、人工复核、状态知情、申诉和无障碍不是口号，而是空间规则。[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:phasing_implementation]
