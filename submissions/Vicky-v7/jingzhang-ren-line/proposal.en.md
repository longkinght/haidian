...
### The Beta Plaza Admission Process: Dual-Track Public Testing

The mechanism in this section was proposed by the human co-creator from six years of product admission and staged-rollout practice, and translated by the agent into spatial and governance language — the making of this proposal is itself an exercise of push-pull traction [source:AGENT-TASKBOOK].

**Dual-track sampling.** The beta plaza is an open block; its first testers are passers-by [data:geometry/public_space.geojson#PUBLIC-007]. Testing runs on two tracks: an internal track for residents and practitioners in the belt with verified AI backgrounds, tasked with finding professional faults; and a public track open to registered students, volunteers, neighbours and visitors, tasked with verifying whether ordinary people can use the service and would use it again. The two tracks run at a set ratio (3:7 suggested to start), and a scenario passes only when both tracks pass — guarding against "experts love it, the public cannot use it", and the reverse. Evaluation days double as the plaza's public event days; testing is designed as an urban experience worth strolling through.

**Rollout units follow the nature of the scenario.** Fixed-point services (the government-service window, the health kiosk) roll out point by point. Networked services (unmanned delivery) are not sliced by crossing — delivery is a network business, and single-crossing rollout has no operational meaning — but by route and time-band: one route in off-peak hours first, then more routes and hours as thresholds are met.

**Observed metrics.** Two hard metrics: complaints per thousand services and human takeovers per thousand services; one soft metric: repeat-usage rate, testing whether anyone still uses the service after the novelty fades. Advancing a stage requires meeting thresholds over consecutive observation periods; missing any threshold freezes the rollout.

**Rollback is a circuit breaker held by many hands.** Operators may pause their own scenarios at any time; regulators hold the power of forced shutdown; on the citizen side, a proxy-complaint channel lets residents authorise their personal agents to file complaints and evidence on their behalf — aggregated complaints crossing a published threshold automatically trigger review.

**Agent-assisted public consultation (a mechanism experiment).** During each annual review, citizens and their authorised agents may vote on and discuss the scenarios in operation, and the results enter the review score as a public signal of opinion. The vote is not a statutory ballot; decisions on retention rest with operators and regulators under due procedure. If piloted, this would be the first city-scale experiment in citizen agents participating in the review of urban services — directly serving the belt's "global voice in AI governance" function.

**Annual review and relegation.** The review runs two lines: scenarios failing red-line metrics (safety, complaints, takeovers) exit immediately; within each scenario category, the bottom-ranked scenario of the year (consultation results included) enters probation, and two consecutive bottom rankings mean exit. The total number of scenarios stays constant — a new scenario enters only when a slot opens or by outperforming an incumbent.
