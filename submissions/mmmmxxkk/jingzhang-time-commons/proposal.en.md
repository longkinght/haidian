...
The latest repository basis note contains two background-only cross-checks. The OSM-mapped Jing-Zhang Railway Heritage Park has 0% intersection with `PROV-SITE-001`, with a nearest distance of about 412.5 metres. At their sampled latitudes, the mapped centre lines of four streets named by the announcement also sit 533–898 metres west of the registered boundary edges, all in the same direction, with a mean of about 667 metres. Neither result establishes that OSM is complete or that the provisional polygon is wrong, so neither justifies silently moving the boundary. Together they establish that every current OSM metric describes only the current provisional polygon and must not be presented as an official park-surroundings baseline. [source:BOUNDARY-BASIS-AUDIT]
...
| Provisional geometry | reproducible, low confidence; park nearest-distance discrepancy about 412.5 m and four named-street centre-line offsets of 533–898 m | internal topology and concept relations | approval, relocation, precise quantities, or actual park inclusion |
