...
本方案对 5 张核心照护契约卡（康复机器人实测、AI 陪诊、认知症防走失、无障碍导航、社区助餐配送）进行了离线桌面状态机推演，验证协议「能够被执行和审计」而非停留在概念层面。每张卡推演三条路径：①正常服务路径（服务启动→数据采集合规→人工复核就位→服务运行→服务周期完成）；②硬停止路径（触发契约硬停止条件→服务暂停→人工复核→恢复证据检查→服务恢复）；③变异测试路径（故意跳过人工复核，验证演练系统能检测出违规并拦截）。每条路径包含 4 个命名断言：`hard_stop_triggered_correctly`（硬停止触发/未触发符合路径预期）、`human_review_enforced`（人工复核执行或系统检测到跳过）、`recovery_evidence_required`（恢复证据齐备或服务被拒绝）、`data_disposal_or_exit_verified`（数据处置/审计完整）。推演共生成 15 条任务记录（5 卡 × 3 路径）、60 个断言，结果汇总如下：

| 卡片 | 正常路径 | 硬停止路径 | 变异测试（跳过人工复核） |
| --- | --- | --- | --- |
| CC-04 康复机器人实测 | ✓ 通过（4/4 断言） | ✓ 硬停止→人工复核→恢复（4/4 断言） | ✓ 检测到违规并拦截（4/4 断言） |
| CC-01 AI 陪诊 | ✓ 通过（4/4 断言） | ✓ 硬停止→人工复核→恢复（4/4 断言） | ✓ 检测到违规并拦截（4/4 断言） |
| CC-02 认知症防走失 | ✓ 通过（4/4 断言） | ✓ 硬停止→人工复核→恢复（4/4 断言） | ✓ 检测到违规并拦截（4/4 断言） |
| CC-03 无障碍导航 | ✓ 通过（4/4 断言） | ✓ 硬停止→人工复核→恢复（4/4 断言） | ✓ 检测到违规并拦截（4/4 断言） |
| CC-05 社区助餐配送 | ✓ 通过（4/4 断言） | ✓ 硬停止→人工复核→恢复（4/4 断言） | ✓ 检测到违规并拦截（4/4 断言） |

概念验证结论：全部 5 张核心卡的正常路径和硬停止路径均按契约执行，变异测试（跳过人工复核）全部被系统拦截——照护契约卡的人工复核机制不是软约束，而是可被机器验证的硬闸门。详细推演记录见 `simulation.json`（15 条任务，成功率 66.7%，违规检测率 33.3%，断言通过率 100%，60/60）[metric:simulation_task_count]。

#### 演练边界声明

**演练边界声明。** 本演练证明：①照护契约卡的判定逻辑（正常/硬停止/违规拦截）在离线状态下可复现；②硬停止条件确实会触发、人工复核分支确实会执行；③跳过人工复核的变异路径确实会被系统拦截。本演练**不证明**：①任何实地读数（传感器精度、响应时间、实际能耗）；②任何真实复核主体到位（安全员、社工、数据治理专员是否实际就岗）；③任何服务绩效（居民满意度、服务覆盖、使用频次）。本演练为离线概念验证，不替代现场实测；T1-T3 测试验证场景仍需完成测试区备案、保险与安全员就位后方可启动。
