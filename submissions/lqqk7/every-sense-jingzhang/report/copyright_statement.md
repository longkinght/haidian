...
| `visual/assets/cjk-webfont.css` · `cjk-webfont-provenance.json` | v9.8 新增：中文离线字体子集及其溯源登记。子集自 Adobe 官方 2.005R 发布物 `09_SourceHanSansSC.zip` 内的 `SourceHanSansSC-Regular.otf` 用 fontTools 生成，只保留本包四份 HTML 与七个注入文案的脚本中实际出现的 1,321 个非 ASCII 字符，输出 woff2 198,180 字节，以 base64 data URI 内嵌于一份 CSS，两份报告页与两份展示页以相对路径共用这一份，全包只存一份字面数据。溯源文件载上游发布物 URL 与 SHA-256、源字体 SHA-256、子集参数全表、按 OFL 保留名条款所作的更名清单、unicode-range 例外的八个码位及其理由，以及覆盖断言的十九项读数 | Source Han Sans SC 2.005R，SIL Open Font License 1.1，© 2014-2025 Adobe，保留名 `Source`。子集属 OFL 定义的 Modified Version，族名已改为 `ESJZ Sans Subset`、PostScript 名改为 `ESJZSansSubset-Regular`，各语言族名记录一并改名，不使用保留名；name ID 0 版权声明与 name ID 13／14 许可声明及许可 URL 在子集内逐字保留，以履行 OFL 1.1 第 2 条。上游发布物内 `LICENSE.txt` 的 SHA-256 与 `font-license-ofl.json` 已登记的 `plain_text_sha256` 实测相同，溯源链自上游发布物到包内许可正本闭合 | `visual/assets/cjk-webfont-provenance.json`；本文件 §四字体台账 |
