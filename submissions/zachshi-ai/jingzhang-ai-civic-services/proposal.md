...
**保障三：回执（Guarantee Receipt）。** 每次服务生成一张可审计回执：AI 做了什么、人工复核了什么、办成/升级/失败状态、投诉渠道。回执的数据边界：仅含事项名称、办理状态与时间戳，不采集办事人身份、材料内容、病历或案件事实；公示的经办人"姓+工号+签字"属于履职公示的职务信息，非个人隐私画像。回执留存 90 天有明确留存依据（季度审计与申诉追溯需要），访问权限限定审计角色，到期匿名化聚合统计后删除明细。回执模式见 `visual/assets/guarantee-receipt.schema.json` [data:visual/assets/guarantee-receipt.schema.json#receipt]。
...
| 数据 | 仅事项名称、办理状态与时间戳（聚合）；不采集身份、材料内容、病历或案件事实；位置数据仅在服务会话内使用不留存；留存 90 天限审计角色访问，到期匿名化后删除明细 |
