(function () {
  ...
  const names = ["accountable_role", "original_channels", "independent_reviewer", "asset_data_recovery_record"];
  const messages = {
  ...
  document.querySelectorAll("button.fault").forEach(button => button.addEventListener("click", () => {
    current = window.JZACCorrection.inject(current, names[Number(button.dataset.index)]);
    show();
  }));
