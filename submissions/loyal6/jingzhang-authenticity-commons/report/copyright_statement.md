...
- 五组 `assets/figures/*.png` 的原始图件从提交 GeoJSON、指标和结构化矩阵确定性绘制；v1.4.2重点片区图来自 `visual/assets/spatial-prototypes.json` 的局部概念网格，不是实测平面或工程无障碍认证。
...
- v1.4.2替换的PDF第5、9、10页使用同一OFL字体的400/700静态实例嵌入；保留页沿用上一版嵌入字体。新的重点区PNG由原创矢量概念图光栅化，不引入第三方图像。
- 本包采用 `COMMUNITY-DISPLAY-ONLY` 声明；仓库与组织方的最终许可条款优先。
- `assets/media/cjk-html-proposal.jpg`与`cjk-html-visual.jpg`保留为72275d8e8版本历史截图，不作为v1.4.2中文HTML视觉验收证据。当前版本仍需人工截图验收。
